...
** Last update Sun May 25 20:11:31 2014 vassil_g
*/
...
void            error_cd(t_mysh_er func_r)
{
  if (func_r & BI_CD_NOPWD)
    printf("%s", BI_CD_NOPWD_MSG);
  else if (func_r & BI_CD_NOOLDPWD)
    printf("%s", BI_CD_NOOLDPWD_MSG);
  else if (func_r & BI_CD_INVAOLDPWD)
    printf("%s", BI_CD_INVAOLDPWD_MSG);
  else if (func_r & BI_CD_NOHOME)
    printf("%s", BI_CD_NOHOME_MSG);
  else if (func_r & BI_CD_INVAHOME)
    printf("%s", BI_CD_INVAHOME_MSG);
  else if (func_r & BI_CD_INVAFOLDER)
    printf("%s", BI_CD_INVAFOLDER_MSG);
}
...
void            msh_error(t_mysh_er func_r, t_sh_token *entry)
{
  ...
  error_cd(func_r);
  if (func_r & MA_ERROR)
    printf("%s", MA_ERROR_MSG);
  if (func_r & BI_UENV_USE)
    printf("%s", BI_UENV_USE_MSG);
  if (func_r & BI_UENV_NF)
    printf("%s : %s", entry->next->str, BI_UENV_NF_MSG);
  if (func_r & BI_SENV_USE)
    printf("%s", BI_SENV_USE_MSG);
  if (func_r & ESTWT_MA_ERROR)
    printf("%s", ESTWT_MA_ERROR_MSG);
  if (func_r & STWT_MA_ERROR)
    printf("%s", STWT_MA_ERROR_MSG);
  if (func_r & EXEC_NF)
    printf("%s : %s", entry->str, EXEC_NF_MSG);
  if (func_r & EXEC_NOPATH)
    printf("%s", EXEC_NOPATH_MSG);
  if (func_r & BI_ENV_USE)
    printf("%s : %s", entry->next->str, BI_ENV_USE_MSG);
  if (func_r & EXEC_FORK_FAILURE)
    printf("%s", EXEC_FORK_FAILURE_MSG);
}
